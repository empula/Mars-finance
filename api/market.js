...
const [fx, cg, cgTop, fear, cgNews] = await Promise.all([
...
safeJson(‘https://api.coingecko.com/api/v3/news?per_page=20’),
]);
...
const newsArr = cgNews?.data || cgNews || [];
const news = Array.isArray(newsArr) ? newsArr.slice(0,20).map(n=>({
title: n.title,
url: n.url,
source: n.author || n.source || ‘CoinGecko’,
cat: ‘crypto’,
ts: n.updated_at || Math.floor(Date.now()/1000)
