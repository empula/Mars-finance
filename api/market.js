...
// Sadece 5 hisse - timeout olmasin
const SYMBOLS = [‘AAPL’,‘MSFT’,‘NVDA’,‘AMZN’,‘TSLA’];

const [fx, cg, cgTop, fear, s1, s2, s3, s4, s5] = await Promise.all([
...
safe(`https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=AAPL&apikey=${AV}`),
safe(`https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=MSFT&apikey=${AV}`),
safe(`https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=NVDA&apikey=${AV}`),
safe(`https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=AMZN&apikey=${AV}`),
safe(`https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=TSLA&apikey=${AV}`),
]);
...
const rawStocks = [s1,s2,s3,s4,s5];
const stockRank = SYMBOLS.map((sym, i) => {
const d = rawStocks[i]?.[‘Global Quote’];
const info = STOCK_INFO[sym];
if(!d || !d[‘05. price’]) return {symbol:sym, …info, price:null, chg:0};
return {
symbol: sym,
name: info.name,
ico: info.ico,
